...
class UnifiedAssetsReportExport implements FromView, WithEvents, ShouldAutoSize
{
    ...
    {
        $query = Asset::whereBetween('expense_date', [$this->start_date, $this->end_date]);

        if (!empty($this->main_category_id) && $this->main_category_id != 0) {
            $query->where('assets_category_id', $this->main_category_id);
        }

        if (!empty($this->sub_category_id) && $this->sub_category_id != 0) {
            $query->where('assets_sub_category_id', $this->sub_category_id);
        }

        $reports_assets = $query->orderBy('expense_date', 'asc')->get();
        $total_assets = $query->clone()->sum('amount');
        $amount_with_tax = $query->clone()->sum('amount_with_tax');
        $tax_amount = $query->clone()->sum('tax_amount');

        return view('backend.pages.Assets.reports_assets_unified_excel', [
