                ...
                <form action="{{route('Discounts.update', $discount->id )}}" method="post">
                    ...
                    <div class="row">
                        ...
                        <div class="form-group col-md-3">
                            ...
                            <select class="form-control select2" name="category_discount_id" required>
                                ...
                                @foreach(App\Models\HR\CategoryDiscounts::all() as $categoryDiscounts)
                                    <option value="{{$categoryDiscounts->id}}" @if($categoryDiscounts->id == $discount->category_discount_id) selected @endif>
                                        @if (app()->getLocale() == 'ar')
                        ...
                        <div class="form-group col-md-3">
                            ...
                            <select class="form-control select2" name="employee_id" required>
                                ...
                                @foreach(App\Models\HR\Employee::all() as $employee)
                                    <option value="{{$employee->id}}" @if($employee->id == $discount->employee_id) selected @endif>
                                        @if (app()->getLocale() == 'ar')
                        ...
                        <div class="form-group col-md-3">
                            ...
                            <input type="text" class="form-control" placeholder="{{trans('discounts.discount_name')}} " name="name" value="{{$discount->name}}" required>
                        </div>

                        <div class="form-group col-md-3">
                            <label for="amount">{{trans('discounts.amount')}}</label>
                            <input type="number" class="form-control" placeholder="{{trans('discounts.amount')}}" name="amount" step="any" value="{{$discount->amount}}" required>
                        </div>
                        <div class="form-group col-md-3">
                            ...
                            <select class="form-control " name="payment_method_id" required>
                                ...
                                @foreach(App\Models\Payment_method::all() as $payment_method)
                                    <option value="{{ $payment_method->id }}" {{ old('payment_method_id', $discount->payment_method_id) == $payment_method->id ? 'selected' : null }}>
                                        @if(app()->getLocale() == 'ar')
                        ...
                        <div class="form-group col-md-3">
                            ...
                            <input type="date" class="form-control" placeholder="{{trans('discounts.date')}}" name="date" value="{{$discount->date}}" required>
                        </div>

                        <div class="form-group col-md-12">
                            <label for="notes">{{trans('discounts.notes')}}</label>
                            <textarea class="form-control" name="notes"  rows="6">{{$discount->notes}}</textarea>
                        </div>

                    </div>

                    <div class="modal-footer">
                        <button type="submit" class="btn btn-success"> {{trans('discounts.Save')}} </button>
                    </div>
