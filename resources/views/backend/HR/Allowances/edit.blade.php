...
@section('page_title')
    {{trans('allowances.edit_allowance')}}
@endsection
                ...
                <form action="{{route('Allowances.update', $allowance->id )}}" method="post">
                    ...
                    <div class="row">
                        ...
                        <div class="form-group col-md-3">
                            ...
                            <select class="form-control select2" name="category_allowance_id" required>
                                ...
                                @foreach(App\Models\HR\CategoryAllowance::all() as $categoryAllowance)
                                    <option value="{{$categoryAllowance->id}}" @if($categoryAllowance->id == $allowance->category_allowance_id) selected @endif>
                                        @if (app()->getLocale() =='ar')
                        ...
                        <div class="form-group col-md-3">
                            ...
                            <select class="form-control select2" name="employee_id" required>
                                ...
                                @foreach(App\Models\HR\Employee::all() as $employee)
                                    <option value="{{$employee->id}}" @if($employee->id == $allowance->employee_id) selected @endif>
                                        @if (app()->getLocale() == 'ar')
                                            {{ $employee->name_ar }}
                                        @else
                                            {{ $employee->name_en }}
                        ...
                        <div class="form-group col-md-3">
                            ...
                            <input type="text" class="form-control" placeholder="{{trans('allowances.allowance_name')}} " name="name" value="{{$allowance->name}}" required>
                        </div>

                        <div class="form-group col-md-3">
                            <label for="amount">{{trans('allowances.amount')}}</label>
                            <input type="number" class="form-control" placeholder="{{trans('allowances.amount')}}" name="amount"step="any" value="{{$allowance->amount}}" required>
                        </div>
                        <div class="form-group col-md-3">
                            ...
                            <select class="form-control " name="payment_method_id" required>
                                ...
                                @foreach(App\Models\Payment_method::all() as $payment_method)
                                    <option value="{{ $payment_method->id }}" {{ old('payment_method_id', $allowance->payment_method_id) == $payment_method->id ? 'selected' : null }}>
                                        @if(app()->getLocale() == 'ar')
                        ...
                        <div class="form-group col-md-3">
                            ...
                            <input type="date" class="form-control" placeholder="{{trans('allowances.date')}}" name="date" value="{{$allowance->date}}" required>
                        </div>

                        <div class="form-group col-md-12">
                            <label for="notes">{{trans('allowances.notes')}}</label>
                            <textarea class="form-control" name="notes"  rows="6">{{$allowance->notes}}</textarea>
                        </div>

                    </div>

                    <div class="modal-footer">
                        <button type="submit" class="btn btn-success"> {{trans('allowances.Save')}} </button>
                    </div>
