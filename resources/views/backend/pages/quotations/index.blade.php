            ...
            <div class="row g-1">
                ...
                <div class="col-md-2">
                    ...
                    <select name="status" class="form-select form-select-sm">
                        <option value="">{{ trans('back.All') }}</option>
                        <option value="pending" {{ request('status') == 'pending' ? 'selected' : '' }}>{{ trans('back.pending') }}</option>
                        <option value="accepted" {{ request('status') == 'accepted' ? 'selected' : '' }}>{{ trans('back.accepted') }}</option>
                        <option value="rejected" {{ request('status') == 'rejected' ? 'selected' : '' }}>{{ trans('back.rejected') }}</option>
                        <option value="converted" {{ request('status') == 'converted' ? 'selected' : '' }}>{{ trans('back.converted') }}</option>
                    </select>
                </div>
                ...
                <div class="col-md-2 d-flex gap-1 align-items-end">
                    ...
                    <a href="{{ route('quotations.index') }}" class="btn btn-success" title="{{ trans('back.refresh') }}">
                        ...
                        <table id="" class="table table-bordered text-center table-sm">
                            ...
                            @foreach($quotations as $key => $quotation)
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        @can('edit_quotation')
                                            @if($quotation->status != 'converted')
                                                <form action="{{ route('quotations.updateStatus', $quotation->id) }}" method="POST" class="d-inline">
                                                    @csrf
                                                    @method('PATCH')
                                                    <select name="status" class="form-select form-select-sm bg-{{ $statusBadge[$quotation->status] ?? 'secondary' }} text-white border-0"
                                                            style="width: auto; display: inline-block;" onchange="this.form.submit()">
                                                        <option value="pending" {{ $quotation->status == 'pending' ? 'selected' : '' }}>{{ trans('back.pending') }}</option>
                                                        <option value="accepted" {{ $quotation->status == 'accepted' ? 'selected' : '' }}>{{ trans('back.accepted') }}</option>
                                                        <option value="rejected" {{ $quotation->status == 'rejected' ? 'selected' : '' }}>{{ trans('back.rejected') }}</option>
                                                    </select>
                                                </form>
                                            @else
                                    ...
                                    <td style="background-color: #fff4e6">{{number_format($quotation->subtotal, 3)}}</td>
                                    ...
                                    <td class="text-center">
                                        <div class="btn-group" role="group">
                                            @can('show_quotation')
                                                <a href="{{ route('quotations.show', $quotation->id) }}" class="btn btn-sm btn-info" title="{{ trans('back.show') }}">
                                            ...
                                            @can('edit_quotation')
                                                @if($quotation->status != 'converted')
                                                    <a href="{{ route('quotations.edit', $quotation->id) }}" class="btn btn-sm btn-primary" title="{{ trans('back.edit') }}">
                                            ...
                                            @if(($quotation->status == 'pending' || $quotation->status == 'accepted') && auth()->user()->can('convert_quotation_to_invoice'))
                                                <a href="#" class="btn btn-sm btn-success" data-bs-toggle="modal" data-bs-target="#convert_quotation{{ $quotation->id }}" title="{{ trans('back.convert_to_invoice') }}">
                                            ...
                                            @can('delete_quotation')
                                                @if($quotation->status != 'converted')
                                                    <a href="#" class="btn btn-sm btn-danger" data-bs-toggle="modal" data-bs-target="#delete_quotation{{ $quotation->id }}" title="{{ trans('back.delete') }}">
