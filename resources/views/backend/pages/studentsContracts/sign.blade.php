    ...
    <div class="container">
        ...
        <div class="row" dir="rtl">
            <div class="col-md-12">
                ...
                <form action="{{ route('studentsContracts.sign', $studentsContract->slug) }}" class="mt-3" method="post" id="products_details" enctype="multipart/form-data">
                    @csrf
                    <div class="row mt-1 justify-content-center">
                        <div class="col-md-4 mb-2">
                            <h5 class="mb-2">توقيع العميل  *</h5>
                            <label class="mb-2"> توقيع العميل <b> (يرجى كتابة الاسم هنا)</b> </label>
                            <div id="sig"></div>
                            <input type="hidden" id="signature64" name="signature" required />
                            <div id="signature-error" class="text-danger pt-2" style="display: none;">يرجى التوقيع قبل الإرسال</div>
                            <button class="btn btn-secondary" type="button" id="clear">مسح</button>
                            <button type="submit" class="btn btn-success">اضغط هنا لحفظ التوقيع وارسال الطلب</button>
                            @error('signature')<div class="text-danger pt-2">{{ $message }}</div>@enderror
                        </div>
                    </div>
                </form>
...
@section('js')
    {{-- التوقيع --}}
    <script type="text/javascript" src="{{ asset('signature/jquery.min.js') }}"></script>
    <link type="text/css" href="{{ asset('signature/jquery-ui.css') }}" rel="stylesheet">
    <script type="text/javascript" src="{{ asset('signature/jquery-ui.min.js') }}"></script>
    <script type="text/javascript" src="{{ asset('signature/jquery.signature.js') }}"></script>
    <link rel="stylesheet" type="text/css" href="{{ asset('signature/jquery.signature.css') }}">
    <script src="{{ asset('signature/jquery.ui.touch-punch.min.js') }}"></script>
    ...
    <script>
        $(function () {
            ...
            $('#clear').click(function (e) {
                ...
                sig.signature('clear');
                $('#signature64').val('');
                $('#signature-error').hide(); // إخفاء الرسالة عند المسح
            });

            // التحقق عند إرسال النموذج
            $('#products_details').submit(function (e) {
                if ($('#signature64').val().trim() === '') {
                    e.preventDefault();
                    $('#signature-error').show();
                    $('html, body').animate({
                        scrollTop: $('#sig').offset().top - 100
                    }, 300);
                } else {
