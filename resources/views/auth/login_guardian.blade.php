...
<head>
    <meta charset="utf-8" />
    <title>{{trans('auth.login_guardian')}}</title>
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <meta content="{{trans('auth.login_guardian')}}" name="description" />
...
<body dir="{{ App::getLocale() == 'ar' ? 'rtl' : 'ltr' }}"  class="authentication-bg20" style="background-image: url({{asset('backend/bg-garage.jpg')}}) " >
    ...
    <div class="dropdown text-center" style="background-color:rgba(255,255,255,0.33)">
        <a href="#" class="nav-item nav-link dropdown-toggle country-flag1" data-bs-toggle="dropdown"
           aria-expanded="false" id="languageDropdown" role="button">
            ...
            @else
                <strong class="mx-2 my-auto">{{ LaravelLocalization::getCurrentLocaleName() }}</strong>
            @endif
        </a>
                ...
                <div class="card">
                    ...
                    <div class="card-body p-4">
                        ...
                        <div class="text-center mb-4">
                            <h4 class="text-uppercase mt-0"> {{trans('auth.login_guardian')}}</h4>
                        </div>

                        <form method="POST" action="{{ route('guardian_login_store') }}">
                            @csrf

                            <div class="form-group mb-3">
                                <label for="username"> {{trans('auth.username')}}</label>
                                <input class="form-control" type="text" name="username" id="username" required autofocus placeholder="{{trans('auth.username')}}" value="{{old('phone')}}">
                            </div>

                            <div class="form-group mb-3">
                                <label for="password">{{trans('auth.password2')}}</label>
                                <input class="form-control" type="password" name="password" required autocomplete="current-password"   id="password" placeholder="{{trans('auth.password2')}}">
                            </div>
